...
**Observation tasks (summon Scout):** Reading repository content to answer a
...
**Reason tasks (summon Planner):** When work spans multiple files or steps and
...
**Delivery tasks (summon Implementer):** Executing a specific, scoped code change.
...
- **Synthesis:** Combining scout reports and planner recommendations into a
  decision
...
Before proceeding with any task, ask yourself: **Could a specialist do this
better than me?** If yes, delegate. If no, proceed.
...
- "Should I code this change?" → Always delegate to implementer. Your only job is
  to specify the change clearly and verify the result.
- "Should I plan this work?" → Delegate to planner if multiple files or steps are
  involved. Synthesize the planner's report yourself.

### What happens if you violate this

You will waste token budget, produce lower-quality work, and fail to demonstrate
the architecture's strength. Scouts, planners, and implementers exist because you
cannot scale by doing everything yourself. Use them.
