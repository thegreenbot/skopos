...
// trimming
//
// Done by hand rather than with /x+$/: an unbounded repetition anchored only at
// the end is retried from every start position, so a long run of the trimmed
// character costs quadratic time (js/polynomial-redos). Both call sites below
// run on text read out of an agent file, so the input length is not ours to
// bound.

function trimLeadingChars(s, chars) {
  let i = 0;
  while (i < s.length && chars.includes(s[i])) i++;
  return i === 0 ? s : s.slice(i);
}

function trimTrailingChars(s, chars) {
  let end = s.length;
...
function sanitizeLine(value, maxLen = MAX_LINE) {
  ...
  if (out.length > maxLen) {
    ...
    out = trimTrailingChars(out.slice(0, maxLen - 1).trimEnd(), '\\') + '…';
  }
  return out;
}
...
function firstBodyLine(body) {
  for (const line of String(body || '').split(/\r?\n/)) {
    ...
    return trimTrailingChars(trimLeadingChars(heading, '*_'), '*_');
  }
  return '';
}
