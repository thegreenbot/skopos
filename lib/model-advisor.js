...
// Advisories for agents skopos does NOT render (the user's own — see
// lib/discovery.js). Their model is whatever their file says, so the wording
// must never imply skopos picked it. Silent when the model is unknown to the
// registry: their tool may well know it, and we don't.
function adviseUserAgents(records) {
  const out = [];
  for (const r of records || []) {
    if (!r.model) continue;
    const capabilities = capabilitiesFor(r.model);
    if (!capabilities) continue;
    const requirements = requirementsFor({ data: { model: r.model, tools: r.tools || [] } });
    if (TOOLUSE_RANK[capabilities.toolUse] < TOOLUSE_RANK[requirements.toolUse]) {
      out.push(`your agent '${r.name}' declares model '${r.model}' (${capabilities.toolUse} tool-use) but lists ${requirements.toolCount} tool(s) suggesting it needs ${requirements.toolUse} — skopos does not change this, the file is yours`);
    }
  }
  return out;
}
...
module.exports = {
  requirementsFor, assess, assessForTarget, adviseInstall, adviseUserAgents, compatibilityMatrix,
};
