...
// preview models) — Skopos just can't advise on it. `agentNames`, when given,
// turns an override naming an agent outside the current plan into an error; it
// comes from the manifest, which this module can't build for itself.
function inspectModels(config, agentNames) {
  const errors = [];
  const warnings = [];
  ...
  for (const target of targets) {
    ...
    for (const [tier, model] of Object.entries(map)) {
      ...
      if (typeof model !== 'string' || !model) errors.push(`${at} must be a model name`);
      else if (!capabilitiesFor(model)) unregistered(at, model);
    }
  }
...
function configWarnings(config) {
  return inspectModels(config).warnings;
}
...
function validateConfig(config, opts = {}) {
  ...
  errors.push(...inspectModels(config, opts.agentNames).errors);
