...
function runInstall(opts) {
  ...
  // Some `models.agents` checks need the plan to run, so they happen here rather
  // than in the pass above. configErrors was empty, so anything this adds is
  // plan-dependent, and none of it can produce a bad render: an override key
  // that matches no agent is inert whether it is a typo or a leftover from a
  // narrowed catalog.agents. Install therefore reports the whole set as
  // warnings — a global install that already has valid config should not be
  // blocked by a dead key — while `skopos config validate` still errors on the
  // typo so it is a real failure somewhere. The two messages stay distinct
  // (see lib/config.js inspectModels), so the warning still says which it is.
  const planNames = { agentNames: plan.agents.map((a) => a.name), knownAgentNames: plan.knownAgents };
  warnings.push(...configLib.validateConfig(config, planNames));
  warnings.push(...plan.warnings);
  warnings.push(...configLib.configWarnings(config, planNames));
  ...
  for (const t of targets) {
    ...
    const body = renderInstructionsBody(plan, config, t);
...
// `skopos config validate` — errors block, warnings don't. The manifest is built
// here so per-agent overrides can be checked against the agents that exist.
function runConfigValidate(opts) {
  const env = opts.env || resolveEnv();
  const skoposRoot = opts.skoposRoot;
  const loaded = configLib.loadConfig(env, skoposRoot);
  ...
  try {
    const plan = buildManifest(skoposRoot, loaded.config, env);
    planNames = { agentNames: plan.agents.map((a) => a.name), knownAgentNames: plan.knownAgents };
  } catch {
    // A config too malformed to plan from still reports its own errors below;
    // the agent-name checks are simply skipped.
    planNames = {};
  }

  return {
    errors: configLib.validateConfig(loaded.config, planNames).concat(loaded.problems),
    warnings: configLib.configWarnings(loaded.config, planNames),
    path: configLib.configPath(env),
...
module.exports = {
  SKOPOS_VERSION,
  runInstall, runVerify, runUninstall, runStatus, runSourcesSync, runConfigValidate,
