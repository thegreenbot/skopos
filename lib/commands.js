...
function runInstall(opts) {
  ...
  // A per-agent model override naming an agent outside the plan is dead config —
  // a typo, or a leftover from a narrowed catalog.agents — and needs the plan to
  // spot, so it is checked here rather than in the pass above. configErrors was
  // empty, so anything this adds is plan-dependent. `skopos config validate`
  // reports it as an error; install only warns, because the key is inert and
  // failing a whole install over it would be disproportionate.
  warnings.push(...configLib.validateConfig(config, { agentNames: plan.agents.map((a) => a.name) }));
  warnings.push(...plan.warnings);
  warnings.push(...configLib.configWarnings(config));
...
function runConfigValidate(opts) {
  ...
  let agentNames;
  try {
    agentNames = buildManifest(skoposRoot, loaded.config, env).agents.map((a) => a.name);
  } catch {
    // A config too malformed to plan from still reports its own errors below;
    // the agent-name check is simply skipped.
    agentNames = undefined;
  }

  return {
    errors: configLib.validateConfig(loaded.config, { agentNames }).concat(loaded.problems),
    warnings: configLib.configWarnings(loaded.config),
