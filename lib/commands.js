...
function runInstall(opts) {
  ...
  // plan-dependent. A key naming an agent that exists nowhere is a typo whose
  // intent silently fails, so it blocks exactly as the first pass does; a key
  // naming an agent `catalog.agents` merely excluded is inert and only warns.
  // `skopos config validate` draws the same line.
  const planNames = { agentNames: plan.agents.map((a) => a.name), knownAgentNames: plan.knownAgents };
  const planErrors = configLib.validateConfig(config, planNames);
